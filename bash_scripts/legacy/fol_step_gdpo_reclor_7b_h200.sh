...
# 7B FOL Step GDPO on Reclor — 2x H200: GPU 0 = judge, GPU 1 = training
# For Paratera: srun -G2 -p gpu_h200 -t 480 bash bash_scripts/fol_step_gdpo_reclor_7b_h200.sh
...
DATA_DIR=/data/home/scyb676/run/work/verl/data/reclor_prompt_v2

# FOL judge env — training connects to local judge via OpenAI-compatible API
export OPENAI_BASE_URL="http://127.0.0.1:${JUDGE_PORT}/v1"
export FOL_MODEL="Qwen3.6-35B-A3B"

LOG_DIR="logs/reclor_7b_h200_$(date +%Y%m%d_%H%M%S)"
mkdir -p $LOG_DIR
...
echo "Starting 7B Reclor training on GPU 1... logs: $LOG_DIR/train.log"
CUDA_VISIBLE_DEVICES=1 python3 -u -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name=qwen7b_step_gdpo_fol_reclor_h200_v1 \
    trainer.default_local_dir=checkpoints/verl-fol/qwen7b_step_gdpo_fol_reclor_h200_v1 \
