#!/bin/bash
# Launch a verl-fol translator (Qwen3.6-35B-A3B). NO speculative decoding:
# on vLLM 0.19 + hybrid GDN (Qwen3.5/3.6) + TP=2 every spec-decode path is
# broken (2026-07-10, verified live + upstream):
...
# Usage: start_translator.sh <gpus> <port> <tag>   e.g. 4,5 4873 t1
# NOTE: no `set -u` (conda activate hooks reference unset vars).
GPUS=${1:?usage: start_translator.sh <gpus> <port> <tag>}
PORT=${2:?usage: start_translator.sh <gpus> <port> <tag>}
TAG=${3:?usage: start_translator.sh <gpus> <port> <tag>}
source /2022533109/liubushi/miniconda3/etc/profile.d/conda.sh
conda activate verl
cd /2022533109/zhouchuyan/verl
TS=$(date +%Y%m%d_%H%M%S)
echo "Starting translator ${TAG} on GPUs ${GPUS} port ${PORT} (no spec decode)"
CUDA_VISIBLE_DEVICES=$GPUS vllm serve /2022533109/zhouchuyan/models/Qwen3.6-35B-A3B \
    ...
    2>&1 | tee "logs/translator_${TAG}_${TS}.log"
