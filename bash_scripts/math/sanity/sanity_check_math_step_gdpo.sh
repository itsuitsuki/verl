#!/bin/bash
# Sanity check for math_step_gdpo_math_combined.sh: 3 training steps,
# tiny val, no checkpoints. Confirms Isabelle pool startup, translator
...
#
# Usage (translator must already be serving at OPENAI_BASE_URL):
...
bash bash_scripts/math/train/math_step_gdpo_math_combined.sh \
