...
# ReClor + GRPO baseline training script.
# Basic prompt version:
#   bash bash_scripts/prepare_reclor_prompt_v2.sh
# (defaults to PROMPT_FILE=logical_reasoning.txt)
...
DATA_NAME=${DATA_NAME:-reclor_prompt_v2}
...
if [ ! -f "$DATA_DIR/train.parquet" ] || [ ! -f "$DATA_DIR/validation.parquet" ]; then
    echo "ERROR: ReClor parquet not found under: $DATA_DIR" >&2
    echo "Please prepare data first (basic prompt):" >&2
    echo "  bash bash_scripts/prepare_reclor_prompt_v2.sh" >&2
    exit 1
fi
...
python3 -u -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name="qwen2.5-7b_grpo_reclor_baseprompt_${DATA_NAME}" \
