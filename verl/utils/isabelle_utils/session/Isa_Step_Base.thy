...
(* HOL-Algebra imports (2026-07-17): the direct-domain steps (group / ring / field
   statements, checked in-locale) run in THIS session since the merged single pool
   replaced the separate HOL-Algebra pools. The session runs quick_and_dirty=false:
   nothing in this theory or in any generated theorem uses sorry, so the kernel
   itself guarantees there is no unchecked proof path on either verification path. *)

(* General trigonometric value and sign facts (2026-07-14, generalized 2026-07-18).
   Value-independent meta-theorems for the trig proof system
   (verl/utils/isabelle_utils/trigonometry.py): given the SIGN of cos a (or sin a)
   as a hypothesis, each lemma turns one admitted trig value (tan a = t,
   sin a = s, cos a = c) into the exact value of another trig application, with
   simp evaluating the possibly irrational constant. div_sqrt_eq rationalises
   A/sqrt B = C to the pure check A^2 = C^2*B, so rational and irrational answers
   close identically. The interval sign lemmas further down feed these hypotheses
   from strict principal-quadrant bounds. *)
...
(* Pythagorean value transfer: one admitted sin (or cos) value plus the OTHER
   function's sign hypothesis yields that function's exact value. The square
   root stays symbolic; the caller supplies a kernel-checked rational candidate
   when one exists. *)

lemma cos_from_sin_cpos:
  fixes a s :: real assumes c: "cos a > 0" and se: "sin a = s"
  shows "cos a = sqrt (1 - s^2)"
proof -
  have sq: "(cos a)^2 = 1 - s^2"
    using se sin_cos_squared_add[of a] by (simp add: algebra_simps)
  have "cos a = sqrt ((cos a)^2)" using c by (simp add: real_sqrt_abs)
  thus ?thesis using sq by simp
qed

lemma cos_from_sin_cneg:
  fixes a s :: real assumes c: "cos a < 0" and se: "sin a = s"
  shows "cos a = - sqrt (1 - s^2)"
proof -
  have sq: "(cos a)^2 = 1 - s^2"
...
lemma sin_from_cos_spos:
  fixes a c :: real assumes s: "sin a > 0" and ce: "cos a = c"
  shows "sin a = sqrt (1 - c^2)"
proof -
  have sq: "(sin a)^2 = 1 - c^2"
    using ce sin_cos_squared_add[of a] by (simp add: algebra_simps)
  have "sin a = sqrt ((sin a)^2)" using s by (simp add: real_sqrt_abs)
  thus ?thesis using sq by simp
qed

lemma sin_from_cos_sneg:
  fixes a c :: real assumes s: "sin a < 0" and ce: "cos a = c"
  shows "sin a = - sqrt (1 - c^2)"
proof -
  have sq: "(sin a)^2 = 1 - c^2"
